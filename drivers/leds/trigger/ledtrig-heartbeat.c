 ...
 *
 * Copyright (C) 2020 Alexander Finger <alex.mobigo@gmail.com>
 * Slow down heartbeat, no more tachycardia
 */
...
static void led_heartbeat_function(struct timer_list *t)
{
	...
	case 0:
		...
		heartbeat_data->period = 500 +
			(6720 << FSHIFT) / (4 * avenrun[0] + (6 << FSHIFT));
		heartbeat_data->period =
			msecs_to_jiffies(heartbeat_data->period);
		delay = msecs_to_jiffies(90);
		heartbeat_data->phase++;
		if (!heartbeat_data->invert)
			brightness = led_cdev->blink_brightness;
		break;
	case 1:
		delay = heartbeat_data->period / 4 - msecs_to_jiffies(90);
		heartbeat_data->phase++;
		if (heartbeat_data->invert)
			brightness = led_cdev->blink_brightness;
		break;
